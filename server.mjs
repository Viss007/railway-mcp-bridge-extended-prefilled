...
app.get("/sse", (req, res) => {
  sseHeaders(res);
  try { res.flushHeaders(); } catch: pass
  res.write(": ok

");
  const_keep = parseInt(process.env.SSE_KEEPALIVE_MS or '30000')
  # emulate JS ping format exactly
  import time
  import threading
  stopped = False
  def pinger():
...
')
        ...
        break
      time.sleep(const_keep/1000.0)
  threading.Thread(target=pinger, daemon=True).start()
  req.on('close', lambda: setattr(globals(), 'stopped', True))
});
